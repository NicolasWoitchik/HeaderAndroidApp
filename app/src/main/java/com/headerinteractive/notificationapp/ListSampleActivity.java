...
import com.headerinteractive.notificationapp.layout.CustomListAdapter;
...
public abstract class ListSampleActivity extends SampleActivity implements AdapterView.OnItemClickListener, CustomListAdapter.ValueChangeListener {
    ...
    private final String TAG = "ListSampleActivity";
    ...
    protected void handleActionIntent(Intent intent) {
        if(intent != null && intent.getExtras() != null) {
            if(intent.hasExtra(MyMceBroadcastReceiver.ACTION_KEY)) {
                String action = intent.getStringExtra(MyMceBroadcastReceiver.ACTION_KEY);
                if (action != null) {
                    if (MyMceBroadcastReceiver.ACTION_SDK_REGISTRATION.equals(action)) {
                        handleSdkRegistration();
                    } else if (MyMceBroadcastReceiver.ACTION_GCM_REGISTRATION.equals(action)) {
    ...
    protected void showInAppIfExists() {
        if(!inAppShown) {
            ...
            if (inAppFragmentId > 0) {
                ...
                values.add("appOpen");
                Log.d(TAG, "Iniciando o Inapp. Valores" +values.toString());
